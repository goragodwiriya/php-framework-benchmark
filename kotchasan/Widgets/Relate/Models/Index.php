	...
	public function getModule($id, $count)
	{
		if ($id > 0) {
			...
			if ($index && $index['relate'] !== '') {
				...
				$qs = implode(' OR ', $qs);
				// ชื่อตาราง
				$table_index = $this->tableWithPrefix('index');
				$table_index_detail = $this->tableWithPrefix('index_detail');
				$table_user = $this->tableWithPrefix('user');
				...
				$query = $this->db()->createQuery()
					->select('Y.id', 'Y.topic', 'Y.alias', 'Y.picture', 'Y.comment_date', 'Y.last_update', 'Y.create_date', 'Y.description', 'Y.comments', 'Y.visited', 'U.status', 'U.id member_id', 'U.displayname', 'U.email')
					->from(array($sql3, 'Y'))
