	...
	public function save(Request $request)
	{
		if ($request->isReferer() && $request->inintSession()) {
			...
			if ($request->post('antispam', '')->toString() !== $request->session($antispamid, null)->toString()) {
				...
				// อ่านข้อมูล
				$index = $this->get($id, $module_id, $index_id);
				if ($index) {
					// login
					$login = Login::isMember();
					// ผู้ดูแล
					$moderator = Gcms::canConfig($login, $index, 'moderator');
					// ผู้ดูแล และ สามารถแสดงความคิดเห็นได้
					$canReply = $moderator && !empty($index['can_reply']);
				...
				if (empty($ret)) {
					...
					if ($id > 0) {
						// แก้ไข
						$this->db()->update($this->tableWithPrefix('board_r'), $id, $post);
						// คืนค่า
						...
						$id = $this->db()->insert($this->tableWithPrefix('board_r'), $post);
						// อัปเดทคำถาม
					...
					$this->db()->update($this->tableWithPrefix('board_q'), $index['id'], $q);
					// เคลียร์ antispam
	...
	private function get($id, $module_id, $index_id)
	{
		if ($id > 0) {
			...
			$index = $this->db()->createQuery()
				...
				->toArray()
				->cacheOn()
				->first('R.member_id', 'Q.id', 'Q.comments', 'R.module_id', 'R.picture', 'Q.can_reply', 'M.module', 'M.config mconfig', 'C.config', 'C.category_id');
		} else {
			...
			$index = $this->db()->createQuery()
				...
				->toArray()
				->cacheOn()
				->first('Q.id', 'Q.comments', 'Q.module_id', 'Q.can_reply', 'M.module', 'M.config mconfig', 'C.config', 'C.category_id');
		}
		if ($index) {
			// config จากโมดูล
