...
class Model extends \Kotchasan\Model
{
	...
	{
		if ($request->isReferer() && $request->inintSession() && preg_match('/(quote|edit|delete|deleting|pin|lock|print|pdf)-([0-9]+)-([0-9]+)-([0-9]+)-(.*)$/', $request->post('id')->toString(), $match)) {
			$action = $match[1];
			$qid = (int)$match[2];
			$rid = (int)$match[3];
			$no = (int)$match[4];
			...
			if ($index === false) {
				...
				if ($action === 'quote') {
					...
					if ($action == 'pin') {
						...
						$this->db()->update($this->tableWithPrefix('board_q'), $qid, array('pin' => $ret['value']));
						$ret['title'] = Language::get($ret['value'] == 1 ? 'Unpin' : 'Pin');
					} elseif ($action == 'lock') {
						$ret['value'] = $index['locked'] == 0 ? 1 : 0;
						$this->db()->update($this->tableWithPrefix('board_q'), $qid, array('locked' => $ret['value']));
					...
					if ($rid > 0) {
						...
						// ลบความคิดเห็น
						$this->db()->delete($this->tableWithPrefix('board_r'), $rid);
						...
						$r = $this->db()->createQuery()
							...
							->order('C.id DESC')
							->toArray()
							->first('C.id', 'C.module_id', 'C.last_update', 'U.id member_id', 'U.status', "(CASE WHEN ISNULL(U.`id`) THEN C.`email` WHEN U.`displayname`='' THEN U.`email` ELSE U.`displayname` END) displayname");
						// อัปเดทคำถาม
						$this->db()->update($this->tableWithPrefix('board_q'), $qid, array(
							'comment_id' => $r ? (int)$r['id'] : 0,
							'commentator' => $r ? $r['displayname'] : '',
						...
						// ลบ
						$this->db()->delete($this->tableWithPrefix('board_q'), $qid);
						$this->db()->delete($this->tableWithPrefix('board_r'), array(array('index_id', $qid), array('module_id', $index['module_id'])));
						if ($action == 'deleting') {
