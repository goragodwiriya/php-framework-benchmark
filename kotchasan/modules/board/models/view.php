	...
	public static function get($module_id, $id)
	{
		if (is_int($module_id) && $module_id > 0) {
			...
			$fields = array(
				...
				"(CASE WHEN ISNULL(U.`id`) THEN I.`email` WHEN U.`displayname`='' THEN U.`email` ELSE U.`displayname` END) displayname",
			);
			...
			if (sizeof($result) == 1) {
				$result[0]['visited'] ++;
				$model->db()->update($model->tableWithPrefix('board_q'), $result[0]['id'], array('visited' => $result[0]['visited']));
