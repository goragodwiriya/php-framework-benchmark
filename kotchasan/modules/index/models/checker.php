...
class Model extends \Kotchasan\Model
{
	...
	public function email()
	{
		...
		if (self::$request->isReferer()) {
			...
			if (!Validator::email($value)) {
				...
				// ตรวจสอบอีเมล์ซ้ำ
				$search = $this->db()->first($this->tableWithPrefix('user'), array('email', $value));
	...
	public function phone()
	{
		...
		if (self::$request->isReferer()) {
			...
			if (!preg_match('/[0-9]{9,10}/', $value)) {
				...
				// ตรวจสอบโทรศัพท์
				$model = new static;
				$search = $model->db()->first($model->tableWithPrefix('user'), array('phone1', $value));
				if ($search && ($id == 0 || $id != $search['id'])) {
	...
	public function idcard()
	{
		...
		if (self::$request->isReferer()) {
			...
			if (!preg_match('/[0-9]{13,13}/', $value)) {
				...
				// ตรวจสอบ idcard
				$model = new static;
				$search = $model->db()->first($model->tableWithPrefix('user'), array('idcard', $value));
				if ($search && ($id == 0 || $id != $search['id'])) {
	...
	public function displayname()
	{
		...
		if (self::$request->isReferer()) {
			...
			if (!empty($value)) {
				// ตรวจสอบ ชื่อเรียก
				$model = new static;
				$search = $model->db()->first($model->tableWithPrefix('user'), array('displayname', $value));
				if ($search && ($id == 0 || $id != $search->id)) {
	...
	public function antispam()
	{
		...
		if (self::$request->isReferer() && self::$request->inintSession()) {
			...
			$value = self::$request->post('value')->toString();
			if (empty($_SESSION[$id]) || $_SESSION[$id] != $value) {
				echo str_replace(':name', Language::get('Antispam'), Language::get('Incorrect :name'));
			}
		}
	}
	...
	public static function alias()
	{
		...
		if (self::$request->isReferer()) {
			...
			$model = new static;
			// ค้นหาชื่อเรื่องซ้ำ
			$search = $model->db()->first($model->tableWithPrefix('index'), array('alias', $value));
			if ($search && ($id == 0 || $id != $search->id)) {
