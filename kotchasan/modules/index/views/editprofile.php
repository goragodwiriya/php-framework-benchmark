...
class View extends \Kotchasan\View
{
	...
	public function render(Request $request)
	{
		if ($login = Login::isMember()) {
			...
			$member_tabs = array_keys(self::$cfg->member_tabs);
			$tab = in_array($tab, $member_tabs) ? $tab : reset($member_tabs);
			$index = (object)array('description' => self::$cfg->web_description);
			// รายการ tabs
			$tabs = array();
			if ($login['fb'] == 1) {
				unset(self::$cfg->member_tabs['password']);
			}
			foreach (self::$cfg->member_tabs AS $key => $values) {
				if ($values[0] != '') {
					if ($key == $tab) {
			...
			if (empty($className)) {
				// FB และแก้ไขรหัสผ่าน
				return createClass('Index\PageNotFound\View')->render();
			} else {
				$template = Template::create('member', 'member', 'main');
				$template->add(array(
					'/{TAB}/' => implode('', $tabs),
					'/{DETAIL}/' => createClass($className)->render()
				));
