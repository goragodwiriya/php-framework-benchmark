...
class View extends \Kotchasan\View
{
	...
	public function render($index)
	{
		...
		foreach ($languages as $item) {
			...
			// detail
			$detail = new CKEditor('textarea', array(
				'id' => 'details_'.$item,
				'itemClass' => 'item',
				'height' => 300,
			...
			$fieldset->appendChild($detail->render());
		}
		// รายละเอียดอื่นๆ
		$fieldset = $form->add('fieldset', array(
			'id' => 'options',
		...
		$fieldset->add('file', array(
			...
			'comment' => str_replace(array('{T}', '{W}', '{H}'), array('jpg'), Language::get('Browse image uploaded, type {T} size {W}*{H} pixel (automatic resize)')),
		...
		$form->script('inintWriteTab("accordient_menu", "'.$tab.'");');
