				...
				if (empty($index)) {
					...
					if ($canWrite) {
						...
						if (in_array($save['alias'], Gcms::$MODULE_RESERVE) || is_dir(ROOT_PATH."modules/$save[alias]") || is_dir(ROOT_PATH."widgets/$save[alias]")) {
							...
							// ค้นหาชื่อเรื่องซ้ำ
							$search = $this->db()->first($this->tableWithPrefix('index'), array(
						...
						if (!$input) {
							...
							if (empty($id)) {
								...
								$index['id'] = $this->db()->insert($this->tableWithPrefix('index'), $save);
							} else {
								// แก้ไข
								$this->db()->update($this->tableWithPrefix('index'), $index['id'], $save);
							}
							// details
							$index_detail = $this->tableWithPrefix('index_detail');
