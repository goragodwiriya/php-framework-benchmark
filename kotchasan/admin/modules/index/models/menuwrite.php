	...
	public static function action()
	{
		...
		if (self::$request->isReferer() && self::$request->inintSession() && $login = Login::isAdmin()) {
			if ($login['email'] == 'demo') {
				...
				$model = new static;
				$table_menus = $model->tableWithPrefix('menus');
				if ($action === 'get' && !empty($parent)) {
					...
					if ($menu->language == '') {
						...
						if ($search === false) {
							...
							$menu->language = $lng;
							$model->db()->update($model->tableWithPrefix('menus'), $menu->id, $menu);
							unset($menu->id);
							// เพิ่มรายการใหม่จากรายการเดิม
							$menu->language = $old_lng;
							$model->db()->insert($model->tableWithPrefix('menus'), $menu);
	...
	public function save()
	{
		...
		if (self::$request->isReferer() && self::$request->inintSession() && $login = Login::isAdmin()) {
			if ($login['email'] == 'demo') {
				...
				$model = new static;
				$table_menu = $model->tableWithPrefix('menus');
				if (!empty($id)) {
