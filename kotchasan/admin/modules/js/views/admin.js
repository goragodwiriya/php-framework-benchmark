function inintMemberStatus(id) {
	...
	function _doAction(c) {
		...
		if (q != '') {
			send('index.php/index/model/memberstatus/action', q, function (xhr) {
				var ds = xhr.responseText.toJSON();
				if (ds) {
					if (ds.data) {
						$G(id).appendChild(ds.data.toDOM());
						_doInintStatusMethod(ds.newId);
	...
	var o = {
		...
		onSave: function (v) {
			...
			req.send('index.php/index/model/memberstatus/action', 'action=' + this.id + '&value=' + encodeURIComponent(v));
			req.hideLoading();
