...
class View extends \Kotchasan\View
{
	...
	public function addBreadcrumb($url, $menu, $tooltip, $class = '')
	{
		...
		$this->breadcrumbs[] = preg_replace($patt, array($class, $url, $tooltip, htmlspecialchars_decode($menu)), $this->breadcrumb_template);
	}
	...
	public function renderHTML($template = null)
	{
		...
		parent::setContents(array(
			// กรอบ login
			'/{LOGIN}/' => method_exists('Index\Login\Controller', 'inint') ? \Index\Login\Controller::inint(Login::isMember()) : '',
			// widgets
			'/{WIDGET_([A-Z]+)(([\s_])(.*))?}/e' => '\Gcms\View::getWidgets(array(1=>"$1",3=>"$3",4=>"$4"))',
			// breadcrumbs
	...
	public static function getWidgets($matches)
	{
		...
		if (isset($matches[3]) && $matches[3] == ' ' && isset($request['module'])) {
