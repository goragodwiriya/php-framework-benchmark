...
class gcms
{
	 ...
	 */
	private static function _filterVars($type, $key)
	{
		if ($type == 'REQUEST') {
			if (isset($_POST[$key])) {
				return $_POST[$key];
	...
	public static function getVars($typies, $keys, $default)
	{
		if (is_array($typies)) {
			...
			foreach (explode(',', $typies) AS $i => $type) {
				$value = gcms::_filterVars($type, $keys[$i]);
				if ($value !== null) {
					break;
				}
			}
	 ...
	 * @return string
	 */
	public static function pagination($totalpage, $page, $url)
	{
		$maxlink = 9;
		...
		return $splitpage == '' ? '<strong title="{LNG_DISPLAY_PAGE} '.$i.'">1</strong>' : $splitpage;
	}
}